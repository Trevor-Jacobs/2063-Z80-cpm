...
;##########################################################################
	;
	ld	a,(bios_disk_last_disk)		;get the last disk number
	ld	b,a					
	ld	a,(bios_disk_current_disk)	;get the current disk number
	cp	b							;Has the disk changed?
	jr	z,.bios_check_sd_block_read
	;
	ld	(bios_disk_last_disk),a		;update last disk number
	jp	.bios_read_block
	;
.bios_check_sd_block_read:	
;##########################################################################	
...
if .rw_debug >= 2
	...
	ld	de,0
	call	.bios_disk_offset	;calculate disk offset based on disk number
	;
...
;##########################################################################
	...
	ld	a,(bios_disk_current_disk)	;get the current disk number
	cp	b							;Has the disk changed?
	jr	z,.bios_check_sd_block_write
	;
	ld	(bios_disk_last_disk),a		;update last disk number
	jp	.bios_write_miss
	;
.bios_check_sd_block_write:	
;##########################################################################	
